		...
		return (
			<div className='form-group'>
				...
				<div className='form-group'>
					...
					<input className='form-control' type='password' value={vm.props.user.password} name='password' onChange={(e)=>{vm.props._handleInputChange(e, 'user'); vm.props._handleInputError(e, 'user')}}/>
					<span className='error'>{vm.props.error.password}</span>
				</div>
				<div className='form-group'>
					<button type='submit' disabled={!vm.props.valid} className={`form-control btn btn-black color-white`}>Log in</button>
				</div>
			</div>
		);
	}
